...
export default function CreateStudentPage() {
  ...
  const derivedAdmissionYear = formData.admissionYear || (formData.sessionYear ? formData.sessionYear.split('-')[0] : '');
  const derivedBatch = (derivedAdmissionYear && selectedCourse?.duration) 
    ? `${derivedAdmissionYear}-${Number(derivedAdmissionYear) + selectedCourse.duration}` 
    : 'Pending Input...';

  let semestersCount = 8;
  if (selectedCourse) {
    semestersCount = selectedCourse.totalSemesters || 8;
  } else if (formData.selectedBranch) {
    const branchCourses = courses.filter(c => typeof c.branchId === 'object' ? c.branchId?._id === formData.selectedBranch : c.branchId === formData.selectedBranch);
    if (branchCourses.length > 0) {
      semestersCount = Math.max(...branchCourses.map(c => c.totalSemesters || 8));
    }
  }
    ...
    <div className="min-h-screen bg-[#F8FAFC] flex font-sans text-slate-800">
              ...
              {selectedCourse && (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mt-12 p-4 bg-white/10 border border-white/20 rounded-2xl backdrop-blur-sm">
                  ...
                  <div className="flex gap-2 mt-3 flex-wrap">
                    <div className="inline-block px-2 py-1 bg-white/20 rounded-md text-[10px] font-bold text-white uppercase">
                      Sem {formData.selectedSemester}
                    </div>
                    {derivedBatch !== 'Pending Input...' && (
                      <div className="inline-block px-2 py-1 bg-white/20 rounded-md text-[10px] font-bold text-white uppercase">
                        Batch {derivedBatch}
                      </div>
                    )}
      ...
      <div className="flex-1 flex flex-col bg-[#F5F6FA]">
                ...
                <div className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm space-y-6">
                  ...
                  <div>
                    ...
                    <select value={formData.selectedProgram} onChange={e => setFormData({...formData, selectedProgram: e.target.value})} className="w-full mt-1 bg-slate-50 border border-slate-100 p-4 rounded-xl font-bold text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 text-sm appearance-none">
                      ...
                      {courses
                        .filter(c => !formData.selectedBranch || (typeof c.branchId === 'object' ? c.branchId?._id === formData.selectedBranch : c.branchId === formData.selectedBranch))
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <select value={formData.selectedSemester} onChange={e => setFormData({...formData, selectedSemester: Number(e.target.value)})} className="w-full mt-1 bg-slate-50 border border-slate-100 p-4 rounded-xl font-bold text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 text-sm appearance-none">
                        {[...Array(semestersCount)].map((_, i) => (
                          <option key={i + 1} value={i + 1}>Semester {i + 1}</option>
                        ))}
                      </select>
                    </div>
                    <div>
                      <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 pl-1">Section (Optional)</label>
                      <input value={formData.selectedSection} onChange={e => setFormData({...formData, selectedSection: e.target.value})} type="text" className="w-full mt-1 bg-slate-50 border border-slate-100 p-4 rounded-xl font-bold text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 text-sm" placeholder="e.g. A" />
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                    ...
                    <div>
                      <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 pl-1">Admission Year (Auto-calc if blank)</label>
                      <input value={formData.admissionYear} onChange={e => setFormData({...formData, admissionYear: e.target.value})} type="text" className="w-full mt-1 bg-slate-50 border border-slate-100 p-4 rounded-xl font-bold text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 text-sm" placeholder="2024" />
                    </div>
                  </div>
                </div>
              </motion.div>
        ...
        <div className="bg-white border-t border-slate-100 p-6 flex justify-between items-center z-10 sticky bottom-0">
          <button 
            onClick={() => step > 0 ? setStep(s => s - 1) : router.push('/office/students')}
