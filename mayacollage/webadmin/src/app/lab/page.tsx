  ...
  return (
    <div className="pb-20 font-sans text-slate-800">
      ...
      <div className="relative overflow-hidden bg-white border border-slate-200 shadow-sm rounded-[2rem] mb-10">
        <div className="absolute top-0 right-0 w-96 h-96 bg-blue-50 rounded-full blur-3xl opacity-60 -translate-y-1/2 translate-x-1/2" />
        <div className="px-8 py-10 relative z-10">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-blue-100/50 border border-blue-200/50 text-blue-700 text-xs font-black uppercase tracking-widest mb-4">
                <Settings className="w-3.5 h-3.5" /> Lab Assistant Portal
              </motion.div>
              ...
              <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="text-slate-500 font-medium mt-2 max-w-xl">
                Monitor laboratory equipment, manage inventory stock, and track issued resources across campus.
              </motion.p>
            </div>
            
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.3 }} className="flex gap-3">
              <Link href="/lab/circulation">
                <button className="px-6 py-3 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 font-bold rounded-xl shadow-sm transition-all flex items-center gap-2 text-sm">
                  <ArrowRightLeft className="w-4 h-4" /> Issue Item
                </button>
              </Link>
              <Link href="/lab/inventory">
                <button className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl shadow-lg shadow-blue-500/30 hover:-translate-y-0.5 transition-all flex items-center gap-2 text-sm">
                  <PackageSearch className="w-4 h-4" /> View Inventory
                </button>
              </Link>
            </motion.div>
          </div>
              ...
              recentActivities.map((tx: any, i) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h4 className="font-bold text-slate-900 group-hover:text-blue-600 transition-colors">{tx.item?.itemName || 'Unknown Item'}</h4>
