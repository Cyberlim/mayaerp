...
const run = async () => {
    try {
        ...
        const mpharmId = new mongoose.Types.ObjectId('6a44bc5b7b077d27cbe52be4'); // Master in pharmacy
        const branchId = new mongoose.Types.ObjectId('6a15797f934eaa3079532001'); // PHARMACY
        const sessionYear = '2025-27';

        const workbook = xlsx.readFile('C:/Users/kdev7/Downloads/maya-collage/mayaerp/mayaerp/admin/data/M.Pharm 2025-27 C.xlsx');
        ...
        for (let i = 1; i < data.length; i++) {
            ...
            // Assuming student name is at index 3 like previous files. Let's check headers on the fly if needed, but assuming same structure.
            if (!row || row.length === 0 || !row[3]) continue;

            const rawName = String(row[3]).trim();
            if (rawName.toLowerCase() === 'student name') continue; // Skip header row just in case
        ...
        console.log(`Successfully uploaded ${inserted} M.Pharm students for 2025-27 batch.`);
        process.exit(0);
