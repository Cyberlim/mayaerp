...
const EXCEL_PATH = 'c:\\Users\\Lalit\\OneDrive\\Desktop\\maya web or backend\\mayacollage\\webadmin\\mayadata\\BBA 2025-28 C.xlsx';
...
const run = async () => {
    try {
        ...
        // Note: The user said they already created Branch "Business Administration" and Course "BBA".
        let branch = await Branch.findOne({ name: { $regex: /Business/i } });
        let course = await Course.findOne({ name: { $regex: /^BBA/i } });

        if (!branch) {
            console.error("❌ Business Administration branch not found!");
            const allBranches = await Branch.find({});
            console.log("Existing branches:", allBranches.map(b => b.name));
            throw new Error("Business Administration branch not found!");
        }

        if (!course) {
            console.error("❌ BBA course not found!");
            const allCourses = await Course.find({});
            console.log("Existing courses:", allCourses.map(c => c.name));
            throw new Error("BBA course not found!");
        }
        ...
        for (let i = 1; i < data.length; i++) { // Start from index 1 to skip header row
            ...
            const studentId = `BBA25${String(importedCount + 1).padStart(3, '0')}`;
        ...
        console.log(`🎉 Successfully imported ${importedCount} BBA (2025) students!`);
        process.exit(0);
    } catch (error) {
