...
const run = async () => {
    try {
        ...
        const bcaCourseId = new mongoose.Types.ObjectId('6a44c1057b077d27cbe52e9a'); // Bachalor in Admintration
        const course = await Course.findById(bcaCourseId);
        if (!course) throw new Error('Course not found');
        const branchId = course.branchId;

        const sessionYear = '2023-26'; // 3 Years

        const workbook = xlsx.readFile('C:/Users/kdev7/Downloads/BCA 2023 C.xlsx');
        ...
        console.log(`Successfully uploaded ${inserted} BCA students for 2023-26 batch.`);
        process.exit(0);
