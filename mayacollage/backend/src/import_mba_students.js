...
const run = async () => {
    try {
        ...
        // Search for Business branch and MBA course
        let branch = await Branch.findOne({ name: { $regex: /Business/i } });
        let course = await Course.findOne({ name: { $regex: /^MBA/i } });
        ...
        if (!course) {
            console.error("❌ MBA course not found!");
            const allCourses = await Course.find({});
            console.log("Existing courses:", allCourses.map(c => c.name));
            throw new Error("MBA course not found!");
        }
        ...
        for (let i = 1; i < data.length; i++) { // Start from index 1 to skip header row
            ...
            const studentId = `MBA24${String(importedCount + 1).padStart(3, '0')}`;
        ...
        console.log(`🎉 Successfully imported ${importedCount} MBA (2024) students!`);
        process.exit(0);
    } catch (error) {
