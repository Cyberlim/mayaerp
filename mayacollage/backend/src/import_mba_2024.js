...
const run = async () => {
    try {
        ...
        const mbaCourseId = new mongoose.Types.ObjectId('6a44c4ee7b077d27cbe5327f'); // Master in Business Admistration
        const course = await Course.findById(mbaCourseId);
        if (!course) throw new Error('Course not found');
        const branchId = course.branchId;

        const sessionYear = '2024-26'; // 2 Years

        const workbook = xlsx.readFile('C:/Users/kdev7/Downloads/MBA 2024-26 C.xlsx');
        ...
        console.log(`Successfully uploaded ${inserted} MBA students for 2024-26 batch.`);
        process.exit(0);
